    ...
    function [] = crClass(varargin)
        ...
        function [] = saveCl(varargin) 
            ...
            for i = 1:size(master,1)
                if master(i,20)==1
                    master_class{i} = 'debris';
                elseif master(i,21) == 1
                    master_class{i} = 'nucleus';
                elseif master(i,22) == 1
                    master_class{i} = 'over';
                elseif master(i,23) == 1
                    master_class{i} = 'under';
                elseif master(i,24) ==1
                    master_class{i} = 'predivision';
                elseif master(i,25) == 1
                    master_class{i} = 'postdivision';
                elseif master(i,26) == 1
                    master_class{i} = 'apoptotic';
                elseif master(i,27) == 1
                    master_class{i} = 'newborn';
                end                
            end
            
            %Now remove all unnecessary columns
                master(:,{'Centroid','BoundingBox','Orientation','PixelIdxList','PixelList','label','bound','debris','nucleus','over','under','predivision','postdivision','apoptotic','newborn'}) = [];
                
                %Run an ensemble fitting for classification
                t = templateTree('MinLeafSize',5);
                rusTree = fitensemble(master,master_class,'RUSBoost',1000,t,...
                    'LearnRate',.1,'nprint',100);
                %run an ensemble fitting for quality of segmentation
...
%%Function to begin tracking cells

%%Function to run assay to measure single cell intensity...
    function [] = cellIntensity(varargin)
    end
